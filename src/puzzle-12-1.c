...
int explore_cave(struct cave *from, struct cave *c) {
	if (from != NULL && strcmp(c->name, "start") == 0) return 0;

	int ret = 0;

	if (strcmp(c->name, "end") == 0) return 1;

	if (!c->big && c->visited > 0) return 0;

	c->visited++;
	for (int i = 0; i < c->link_count; i++) {
		ret += explore_cave(c, c->links[i]);
	}
	c->visited--;
...
int main(int argc, char *argv[]) {
	...
	int ret = explore_cave(NULL, start);
	printf("%d\n", ret);
	return 0;
}
